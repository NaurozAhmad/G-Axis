...
class cnews extends cTable {
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			return "newsadd.php?" . $this->UrlParm($parm);
		else
			return "newsadd.php";
	}
	...
	function GetRecordKeys() {
		...
		if (isset($_POST["key_m"])) {
			...
			$arKeys[] = $isPost ? ew_StripSlashes(@$_POST["news_id"]) : ew_StripSlashes(@$_GET["news_id"]); // news_id
   ...
   // Common render codes
	...
	function RenderEditRow() {
		...
		$this->news_description->EditValue = $this->news_description->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->news_description->Exportable) $Doc->ExportCaption($this->news_description);
				} else {
					if ($this->news_id->Exportable) $Doc->ExportCaption($this->news_id);
					if ($this->news_date->Exportable) $Doc->ExportCaption($this->news_date);
				}
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->news_description->Exportable) $Doc->ExportField($this->news_description);
					} else {
						if ($this->news_id->Exportable) $Doc->ExportField($this->news_id);
						if ($this->news_date->Exportable) $Doc->ExportField($this->news_date);
					}
