...
$category_list = NULL; // Initialize page object first

class ccategory_list extends ccategory {
	...
	function __construct() {
		...
		$this->AddUrl = "categoryadd.php?" . EW_TABLE_SHOW_DETAIL . "=";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "categorydelete.php";
		$this->MultiUpdateUrl = "categoryupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fcategorylistsrch";
	...
	function GetFilterList() {
		...
		$sFilterList = ew_Concat($sFilterList, $this->category_name->AdvancedSearch->ToJSON(), ","); // Field category_name
		$sFilterList = ew_Concat($sFilterList, $this->category_description->AdvancedSearch->ToJSON(), ","); // Field category_description
		if ($this->BasicSearch->Keyword <> "") {
	...
	function BasicSearchSQL($arKeywords, $type) {
		$sWhere = "";
		$this->BuildBasicSearchSQL($sWhere, $this->category_name, $arKeywords, $type);
		$this->BuildBasicSearchSQL($sWhere, $this->category_description, $arKeywords, $type);
		return $sWhere;
	}
	...
	function SetupListOptions() {
		...
		// "detail_sub_category"
		$item = &$this->ListOptions->Add("detail_sub_category");
		$item->CssStyle = "white-space: nowrap;";
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fcategorylist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fcategorylistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($category_list)) $category_list = new ccategory_list();
...
var CurrentForm = fcategorylist = new ew_Form("fcategorylist", "list");
fcategorylist.FormKeyCountName = '<?php echo $category_list->FormKeyCountName ?>';

// Form_CustomValidate event
fcategorylist.Form_CustomValidate = 
...
fcategorylist.ValidateRequired = true;
<?php } else { ?>
fcategorylist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
// Form object for search

var CurrentSearchForm = fcategorylistsrch = new ew_Form("fcategorylistsrch");
</script>
<script type="text/javascript">
...
<?php
	$bSelectLimit = $category_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($category_list->TotalRecs <= 0)
			$category_list->TotalRecs = $category->SelectRecordCount();
	} else {
		if (!$category_list->Recordset && ($category_list->Recordset = $category_list->LoadRecordset()))
			$category_list->TotalRecs = $category_list->Recordset->RecordCount();
	}
	$category_list->StartRec = 1;
	if ($category_list->DisplayRecs <= 0 || ($category->Export <> "" && $category->ExportAll)) // Display all records
		$category_list->DisplayRecs = $category_list->TotalRecs;
	if (!($category->Export <> "" && $category->ExportAll))
		$category_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$category_list->Recordset = $category_list->LoadRecordset($category_list->StartRec-1, $category_list->DisplayRecs);

	// Set no record found message
	if ($category->CurrentAction == "" && $category_list->TotalRecs == 0) {
		if ($category_list->SearchWhere == "0=101")
			$category_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$category_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$category_list->RenderOtherOptions();
?>
<?php if ($category->Export == "" && $category->CurrentAction == "") { ?>
<form name="fcategorylistsrch" id="fcategorylistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($category_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fcategorylistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="category">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($category_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($category_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $category_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($category_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($category_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($category_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($category_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<form name="fcategorylist" id="fcategorylist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($category_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $category_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="category">
<div id="gmp_category" class="<?php if (ew_IsResponsiveLayout()) { echo "table-responsive "; } ?>ewGridMiddlePanel">
<?php if ($category_list->TotalRecs > 0) { ?>
<table id="tbl_categorylist" class="table ewTable">
...
// Header row
$category_list->RowType = EW_ROWTYPE_HEADER;
...
<?php if ($category->category_name->Visible) { // category_name ?>
	<?php if ($category->SortUrl($category->category_name) == "") { ?>
		<th data-name="category_name"><div id="elh_category_category_name" class="category_category_name"><div class="ewTableHeaderCaption"><?php echo $category->category_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="category_name"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $category->SortUrl($category->category_name) ?>',1);"><div id="elh_category_category_name" class="category_category_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $category->category_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($category->category_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($category->category_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
        </div></div></th>
	<?php } ?>
<?php } ?>		
<?php
...
if ($category_list->Recordset && !$category_list->Recordset->EOF) {
	...
	$bSelectLimit = $category_list->UseSelectLimit;
	if (!$bSelectLimit && $category_list->StartRec > 1)
...
$category->RowType = EW_ROWTYPE_AGGREGATEINIT;
$category->ResetAttrs();
$category_list->RenderRow();
while ($category_list->RecCnt < $category_list->StopRec) {
	$category_list->RecCnt++;
	if (intval($category_list->RecCnt) >= intval($category_list->StartRec)) {
...
?>
	<?php if ($category->category_name->Visible) { // category_name ?>
		<td data-name="category_name"<?php echo $category->category_name->CellAttributes() ?>>
<span id="el<?php echo $category_list->RowCnt ?>_category_category_name" class="category_category_name">
<span<?php echo $category->category_name->ViewAttributes() ?>>
<?php echo $category->category_name->ListViewValue() ?></span>
</span>
<a id="<?php echo $category_list->PageObjName . "_row_" . $category_list->RowCnt ?>"></a></td>
	<?php } ?>
<?php
...
<?php if (!isset($category_list->Pager)) $category_list->Pager = new cPrevNextPager($category_list->StartRec, $category_list->DisplayRecs, $category_list->TotalRecs) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $category_list->PageUrl() ?>start=<?php echo $category_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($category_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $category_list->PageUrl() ?>start=<?php echo $category_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $category_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($category_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $category_list->PageUrl() ?>start=<?php echo $category_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($category_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $category_list->PageUrl() ?>start=<?php echo $category_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
