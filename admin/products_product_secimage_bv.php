...
$products_product_secimage_blobview = NULL; // Initialize page object first

class cproducts_product_secimage_blobview extends cproducts {
	...
	// Page object name
	var $PageObjName = 'products_product_secimage_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('products', 'x_product_secimage');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('product_secimage');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('product_secimage')));
				if (trim(strval($this->Recordset->fields('product_secimage'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('product_secimage') . "\"");
				}
...
// Create page object
if (!isset($products_product_secimage_blobview)) $products_product_secimage_blobview = new cproducts_product_secimage_blobview();

// Page init
$products_product_secimage_blobview->Page_Init();

// Page main
$products_product_secimage_blobview->Page_Main();
?>
<?php
$products_product_secimage_blobview->Page_Terminate();
?>
