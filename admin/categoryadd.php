...
class ccategory_add extends ccategory {
	...
	function ValidateForm() {
		...
		if (in_array("sub_category", $DetailTblVar) && $GLOBALS["sub_category"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($bInsertRow) {
			...
			if ($AddRow) {
				...
				$this->category_id->setDbValue($conn->Insert_ID());
				$rsnew['category_id'] = $this->category_id->DbValue;
			}
		} else {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("sub_category", $DetailTblVar) && $GLOBALS["sub_category"]->DetailAdd) {
				$GLOBALS["sub_category"]->category_id->setSessionValue($this->category_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["sub_category_grid"])) $GLOBALS["sub_category_grid"] = new csub_category_grid(); // Get detail page object
				$AddRow = $GLOBALS["sub_category_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["sub_category"]->category_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("sub_category", $DetailTblVar)) {
				...
				if ($GLOBALS["sub_category_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["sub_category_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["sub_category_grid"]->CurrentMode = "add";
					$GLOBALS["sub_category_grid"]->CurrentAction = "gridadd";
...
var CurrentForm = fcategoryadd = new ew_Form("fcategoryadd", "add");
...
?>
<form name="fcategoryadd" id="fcategoryadd" class="<?php echo $category_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
...
<script type="text/javascript">
ew_CreateEditor("fcategoryadd", "x_category_description", 35, 4, <?php echo ($category->category_description->ReadOnly || FALSE) ? "true" : "false" ?>);
</script>
</span>
...
<?php
	if (in_array("sub_category", explode(",", $category->getCurrentDetailTable())) && $sub_category->DetailAdd) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $category_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
</form>
<script type="text/javascript">
