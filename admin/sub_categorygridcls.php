...
$sub_category_grid = NULL; // Initialize page object first

class csub_category_grid extends csub_category {
	...
	// Page object name
	var $PageObjName = 'sub_category_grid';

	// Grid form hidden field names
	var $FormName = 'fsub_categorygrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["sub_category"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function LoadDefaultValues() {
		...
		$this->scat_picture->Upload->Index = $this->RowIndex;
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->scat_id->CurrentValue = strval($arKeys[0]); // scat_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "category") {
			...
			if ($GLOBALS["category"]->EventCancelled) $this->EventCancelled = TRUE;
		}
