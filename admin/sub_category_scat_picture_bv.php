...
$sub_category_scat_picture_blobview = NULL; // Initialize page object first

class csub_category_scat_picture_blobview extends csub_category {
	...
	// Page object name
	var $PageObjName = 'sub_category_scat_picture_blobview';
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('sub_category', 'x_scat_picture');
		...
		if ($this->Recordset = $conn->Execute($sSql)) {
			if (!$this->Recordset->EOF) {
				if (ob_get_length())
					ob_end_clean();
				$objBinary->Value = $this->Recordset->fields('scat_picture');
				...
				if (strpos(ew_ServerVar("HTTP_USER_AGENT"), "MSIE") === FALSE)
					header("Content-type: " . ew_ContentType(substr($data, 0, 11), $this->Recordset->fields('scat_picture')));
				if (trim(strval($this->Recordset->fields('scat_picture'))) <> "") {
					header("Content-Disposition: attachment; filename=\"" . $this->Recordset->fields('scat_picture') . "\"");
				}
...
// Create page object
if (!isset($sub_category_scat_picture_blobview)) $sub_category_scat_picture_blobview = new csub_category_scat_picture_blobview();

// Page init
$sub_category_scat_picture_blobview->Page_Init();

// Page main
$sub_category_scat_picture_blobview->Page_Main();
?>
<?php
$sub_category_scat_picture_blobview->Page_Terminate();
?>
